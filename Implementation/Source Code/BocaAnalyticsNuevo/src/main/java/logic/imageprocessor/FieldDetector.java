...
class FieldDetector extends OpencvDetector {
	...
	private Mat greenMask(Mat image) {
		...
		Scalar alfaMin = new Scalar(Constants.GREEN - Constants.SENSITIVITY, Constants.SV, Constants.SV);
	...
	private Mat bwareopen(Mat image) {
		...
		List<MatOfPoint> contours = new ArrayList<>();// all contorus are saved
														// here
		List<MatOfPoint> littleContours = new ArrayList<>();// little contours
															// are saved here
	...
	private Mat removeScore(Mat image) {
		return (Mat) processor.drawRectangle(image, Constants.SCOREPOINT1, Constants.SCOREPOINT2, Constants.BLACK);
	}
}
